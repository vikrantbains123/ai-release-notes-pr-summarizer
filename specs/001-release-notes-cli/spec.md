...
- **FR-019**: The system MUST require the user to explicitly supply `--version` and `--release-name` whenever `--publish` is used (no silent defaults for either); `--rc-branch` is optional and may be omitted. The commit SHA in the Release Identity header (FR-011) MUST always be auto-resolved from the window's end reference/commit, never user-supplied.
