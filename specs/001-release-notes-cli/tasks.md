...
- [ ] T012 [P] Implement `release_identity.py` in `src/ai_release_notes/release_identity.py`: takes `release_name`/`version`/`rc_branch` as explicit inputs (FR-019) and `commit_sha` auto-resolved from the Time Window's end reference, exposes the version-based matching key — makes T011 pass
...
- [ ] T014 [P] Implement `cost.py` in `src/ai_release_notes/cost.py`: static per-model pricing table, `compute_cost(usage)` (6-decimal precision), a display-formatting helper (4-decimal rounding), `record_usage(path, record)` — makes T013 pass
...
- [ ] T027 [US2] Extend `cli.py` with `--from-ref`/`--to-ref` flags, the mutual-exclusivity check against `--since`/`--until`, and validation that both ref flags are given together or neither — makes T026 pass; depends on T022, T025
